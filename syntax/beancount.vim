...
syn match beanAccount "\v[[:upper:]][[:upper:][:lower:][:digit:]\-]*(:[[:upper:][:digit:]][[:upper:][:lower:][:digit:]\-]*)+" contained
...
syn match beanDate "^\v\d{4}[-/]\d{2}[-/]\d{2}" skipwhite
            \ nextgroup=beanOpen,beanTxn,beanClose,beanNote,beanBalance,beanEvent,beanPad,beanPrice,beanCommodity,beanCustom
...
syn region beanCommodity matchgroup=beanKeyword start="commodity" end="$" contained
            \ keepend contains=beanCurrency
